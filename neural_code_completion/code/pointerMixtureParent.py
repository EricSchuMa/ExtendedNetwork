...
class PMNInput(object):
    ...
    def __init__(self, config, data, name=None, FLAGS=None):
        ...
        self.input_dataN, self.targetsN, self.input_dataT, self.targetsT,\
            self.epoch_size, self.eof_indicator, self.input_dataP = \
...
class PMN(object):
    ...
    def __init__(self, is_training, config, input_, FLAGS):
        ...
        with tf.device("/cpu:0"):
            ...
            inputsP = tf.nn.embedding_lookup(embeddingN, input_.input_dataP)  # parent information
        ...
        inputsPL = tf.concat([inputsP, inputsL], 2)  # concatenate the parent and type information
        if is_training and config.keep_prob < 1:
            inputs = tf.nn.dropout(inputs, config.keep_prob)

        outputs = []  # store hidden state at each time_step
        attentions = []  # store context attention vector at each time_step
        parents = []  # store parent at each time_step
